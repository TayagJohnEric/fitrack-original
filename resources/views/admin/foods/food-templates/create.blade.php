...
@section('title', 'Create Food Suggestion Template')

@section('content')
<div class="min-h-screen bg-white">
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Create Food Suggestion</h1>
            <p class="mt-1 text-sm text-gray-500">Fill in the details to add a new food suggestion template.</p>
        </div>
        ...
        <form action="{{ route('admin.food-templates.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="target_meal_type" required class="mt-1 w-full border rounded px-3 py-2">
                    @foreach(['Breakfast', 'Lunch', 'Dinner', 'Snack', 'Any'] as $type)
                        <option value="{{ $type }}" {{ old('target_meal_type') == $type ? 'selected' : '' }}>
                            {{ $type }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="min_experience_level_id" class="mt-1 w-full border rounded px-3 py-2">
                    ...
                    @foreach($experienceLevels as $level)
                        <option value="{{ $level->id }}" {{ old('min_experience_level_id') == $level->id ? 'selected' : '' }}>
                            {{ $level->name }}
                        </option>
                    @endforeach
                </select>
